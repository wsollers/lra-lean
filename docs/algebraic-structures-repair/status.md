...
`as-24`, and `as-25` have now completed their full queue-local audit/examples/constructions pass
as well; the canonical queue therefore continues at the next unfinished subject.

**Next step:** continue the canonical per-subject queue at `as-26`
`LinearlyOrderedGroup`'s full reorg/audit/examples pass. Its prerequisite
(`OrderedGroup`) is already done.
