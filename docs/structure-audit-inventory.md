...
> **Architecture correction (2026-09-07).** This audit originally propagated
> the superseded rule that concrete backends belong in `Realizations/`.
> Concrete backends are owned by `Constructions/<Construction>/`; proofs that
> they realize interfaces are construction-local satisfaction certificates.
> Historical findings below have been restated using that distinction.
...
| `Interface` / `Interface/ModelTheory` / `Interface/UniversalAlgebra` define generic targets; `Constructions/<Construction>` owns concrete backends; construction-local `Satisfies/` or `Satisfy_<Target>.lean` files certify the targets. | `docs/architecture/repository-architecture.md` §"Interfaces, Constructions, And Satisfaction" |
| `Laws` remains explicit at both levels: generic consequences at subject/interface level and construction-specific proofs beneath each construction. | `docs/architecture/repository-architecture.md` §"Concept Roles", §"Construction Pipeline" |
...
| Migration matrix for interfaces, constructions, and construction-local satisfaction families with a recommended chunk order. | `docs/interface-model-standardization-inventory.md` |
...
| ISS-010 | construction-placement | high | `LRA/EuclideanSpace/Model/{Euclidean3Space,Euclidean4Space,EuclideanN,CartesianPlane,RealLine}.lean` | `EuclideanSpace/Interface/ModelTheory/{LStructure,Theory,Model}.lean` already holds the real model-theory triplet; the `Model/` folder actually holds concrete coordinate-space constructions, imported by `LRA/EuclideanSpace.lean` and by the MetricSpace Euclidean backend | Move them beneath a named `LRA/EuclideanSpace/Constructions/<Construction>/` owner; keep any interface-realization theorem in that construction's satisfaction layer and update consumers | true | true | false | Requires construction-boundary classification before moving; it is not merely a folder rename |
| ISS-011 | modeltheory-placement | medium | `LRA/Analysis/MetricSpace/Construction/ModelTheory.lean` | File body forwards to the mistakenly introduced `LRA.Analysis.MetricSpace.Realizations.Euclidean` path | Replace both legacy locations with a named `Constructions/Euclidean/` owner and its satisfaction certificate once import-graph consumers are mapped | true | false | false | The old pure-deletion recommendation was incomplete because it left the backend under the wrong owner |
| ISS-012 | modeltheory-placement | high | `LRA/Analysis/MeasureTheory/AlgebraOfSets/Construction/ModelTheory/{SetRings,SetAlgebras,BooleanAlgebras}/Signature.lean` and `.../ModelTheory.lean` | Directory still present; matches the standardization inventory's Family C and Family D | Classify each file as either construction-specific material (→ a named `Constructions/<Construction>/` owner, with satisfaction proofs beneath it) or genuine formal interface logic (→ `Interface/ModelTheory/`) | true | true | false | Still the largest Family C footprint; classification must precede movement |
...
3. **Resolve the two live-duplication placement issues with contained blast
   radius**: ISS-010 (`EuclideanSpace/Model` → named `Constructions`, known
...
`PurposeAndArchitecture.md`'s NumberSystems three-layer rule and the corrected
generic interface/construction/satisfaction distinction compose as follows — confirmed against the
...
There is no separate `Realizations/` folder in `NumberSystems`, and there
should not be a separate one in `Analysis` or `Topology` either.
`Constructions/<Construction>/` owns the concrete mathematics, while
construction-local satisfaction modules carry the interface certificates.
