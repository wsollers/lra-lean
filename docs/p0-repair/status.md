...
| p0-06 | 1 | `in_progress` | 2026-08-22: spec and verify are frozen on disk; the active integration router still needs placeholder replacement in `Partitions`, `DarbouxIntegral`, `MeasureZero`, and `RiemannStieltjes`, plus theorem-surface cleanup in `HenstockKurzweil` and `McShaneIntegral` | Launch implement task | `main-thread` | `main-thread` | - | LRA/Analysis/Integration | LRA.Analysis.Integration | - |
