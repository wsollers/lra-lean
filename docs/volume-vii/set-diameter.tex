...
\begin{gathered}
  ...
  \texttt{\ \ \{ r : Real | exists x : X, x in S and}\\
  \texttt{\ \ \ \ exists y : X, y in S and r = dist x y \}.}
\end{gathered}
\]
...
\begin{gathered}
  ...
  \texttt{\ \ \ \ diameterSet A subset diameterSet B := by}\\
...
\texttt{r} of \texttt{diameterSet A}. By definition, it is
\texttt{dist x y} for some \texttt{x} and \texttt{y} in \texttt{A}. The
...
the same real number \texttt{r} belongs to \texttt{diameterSet B}.
...
\begin{gathered}
  ...
  \texttt{\ \ \ \ \{A B : Set X\}}\\
  \texttt{\ \ \ \ (set\_inclusion : A subset B)}\\
  \texttt{\ \ \ \ (A\_diameterSet\_nonempty : (diameterSet A).Nonempty)}\\
  \texttt{\ \ \ \ (B\_diameterSet\_bddAbove : BddAbove (diameterSet B)) :}\\
  \texttt{\ \ \ \ diameter A <= diameter B := by}\\
