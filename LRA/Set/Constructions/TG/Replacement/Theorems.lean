...
Predicate logic (unfolded):
  ...
  ∀ {A : LRA.Set.Constructions.TG.Set} {relation : LRA.Set.Constructions.TG.Set → LRA.Set.Constructions.TG.Set → Prop} {B C : LRA.Set.Constructions.TG.Set}, (∀ (y : LRA.Set.Constructions.TG.Set), LRA.Set.instMembershipTGSet.1 B y ↔ Exists fun x => (LRA.Set.instMembershipTGSet.1 A x ∧ relation x y) ∧ ∀ (y : LRA.Set.Constructions.TG.Set), LRA.Set.instMembershipTGSet.1 C y ↔ Exists fun x => (LRA.Set.instMembershipTGSet.1 A x ∧ relation x y)) → C = B
...
Predicate logic (unfolded):
  ...
  ∀ (A : LRA.Set.Constructions.TG.Set) (relation : LRA.Set.Constructions.TG.Set → LRA.Set.Constructions.TG.Set → Prop) (functional : ∀ (x : LRA.Set.Constructions.TG.Set), LRA.Set.instMembershipTGSet.1 A x → Exists fun y => (relation x y ∧ ∀ (other : LRA.Set.Constructions.TG.Set), relation x other → other = y)) (y : LRA.Set.Constructions.TG.Set), LRA.Set.instMembershipTGSet.1 (Classical.indefiniteDescription (LRA.Set.Constructions.TG.IsReplacementImageOf A relation) ⋯).1 y ↔ Exists fun x => (LRA.Set.instMembershipTGSet.1 A x ∧ relation x y)
