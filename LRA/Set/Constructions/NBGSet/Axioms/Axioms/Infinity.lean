...
Predicate logic:
  ...
  exists A : NBGSet, exists empty ∈ NBGSet, empty ∈ A ∧ forall w : NBGSet, w ∉ empty ∧ forall x : NBGSet, x ∈ A -> exists successor : NBGSet, successor ∈ A ∧ forall w : NBGSet, w ∈ successor <-> w ∈ x ∨ w = x end LRA.Set.Constructions.NBGSet.Axioms

Predicate logic (unfolded):

  Exists fun A => (Exists fun empty => (LRA.Set.instMembershipNBGSet.1 A empty ∧ ∀ (w : LRA.Set.Constructions.NBGSet), LRA.Set.instMembershipNBGSet.1 empty w → False) ∧ ∀ (x : LRA.Set.Constructions.NBGSet), LRA.Set.instMembershipNBGSet.1 A x → Exists fun successor => (LRA.Set.instMembershipNBGSet.1 A successor ∧ ∀ (w : LRA.Set.Constructions.NBGSet), LRA.Set.instMembershipNBGSet.1 successor w ↔ Or (LRA.Set.instMembershipNBGSet.1 x w) (w = x)))
