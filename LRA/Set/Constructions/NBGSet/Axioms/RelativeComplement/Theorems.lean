...
Predicate logic (unfolded):
  ...
  ∀ (A B x : LRA.Set.Constructions.NBGSet.Axioms.Set), LRA.Set.instMembershipNBGSet.1 (Classical.indefiniteDescription (LRA.Set.Constructions.NBGSet.Axioms.IsSeparatedSubset A fun x => ¬ LRA.Set.instMembershipNBGSet.mem B x) ⋯).1 x ↔ (LRA.Set.instMembershipNBGSet.1 A x ∧ LRA.Set.instMembershipNBGSet.1 B x → False)
