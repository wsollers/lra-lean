...
Predicate logic (unfolded):
  ...
  ∀ (A B x : LRA.Set.TG.Set), LRA.Set.instMembershipTGSet.1 (Classical.indefiniteDescription (LRA.Set.TG.IsSeparatedSubset A fun x => LRA.Set.instMembershipTGSet.mem B x) ⋯).1 x ↔ (LRA.Set.instMembershipTGSet.1 A x ∧ LRA.Set.instMembershipTGSet.1 B x)
