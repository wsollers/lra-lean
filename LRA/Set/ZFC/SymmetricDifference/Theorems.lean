...
Predicate logic (unfolded):
  ...
  ∀ (A B x : LRA.Set.ZFC.Set), LRA.Set.instMembershipZFCSet.1 (Classical.indefiniteDescription (LRA.Set.ZFC.IsUnionOf (LRA.Set.ZFC.PairSet (LRA.Set.ZFC.TheRelativeComplement A B) (LRA.Set.ZFC.TheRelativeComplement B A))) ⋯).1 x ↔ Or ((LRA.Set.instMembershipZFCSet.1 A x ∧ LRA.Set.instMembershipZFCSet.1 B x → False)) ((LRA.Set.instMembershipZFCSet.1 B x ∧ LRA.Set.instMembershipZFCSet.1 A x → False))
