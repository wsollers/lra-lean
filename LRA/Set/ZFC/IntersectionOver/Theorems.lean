...
Predicate logic (unfolded):
  ...
  ∀ (A x : LRA.Set.ZFC.Set), (Exists fun B => LRA.Set.instMembershipZFCSet.1 A B) → LRA.Set.instMembershipZFCSet.1 (Classical.indefiniteDescription (LRA.Set.ZFC.IsSeparatedSubset (LRA.Set.ZFC.TheUnionOver A) fun x => ∀ (B : LRA.Set.ZFC.Set), LRA.Set.instMembershipZFCSet.mem A B → LRA.Set.instMembershipZFCSet.mem B x) ⋯).1 x ↔ ∀ (B : LRA.Set.ZFC.Set), LRA.Set.instMembershipZFCSet.1 A B → LRA.Set.instMembershipZFCSet.1 B x
