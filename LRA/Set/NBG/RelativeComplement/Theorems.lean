...
Predicate logic (unfolded):
  ...
  ∀ (A B x : LRA.Set.NBG.Set), LRA.Set.instMembershipNBGSet.1 (Classical.indefiniteDescription (LRA.Set.NBG.IsSeparatedSubset A fun x => ¬ LRA.Set.instMembershipNBGSet.mem B x) ⋯).1 x ↔ (LRA.Set.instMembershipNBGSet.1 A x ∧ LRA.Set.instMembershipNBGSet.1 B x → False)
