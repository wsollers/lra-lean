...
/-- `RationalNumbersTheory signature` means `signature` satisfies the
densely-ordered-field laws expected of ℚ: `add`/`multiply` are each
associative and commutative, `zero`/`one` are two-sided identities, `neg` is
a two-sided additive inverse, `zero` absorbs `multiply`, `multiply`
distributes over `add`, `0 ≠ 1`, every nonzero element has a multiplicative
inverse via `inv` (and `inv 0 = 0`), `le` is a total order compatible with
`StrictOrder`, both operations respect that order, and the order is dense (a
strict inequality always has something strictly between). This is the
model-theoretic restatement of the laws bundled by `RationalModel`, stated
directly over the raw operations and relations carried by `signature` rather
than over a typeclass instance.
