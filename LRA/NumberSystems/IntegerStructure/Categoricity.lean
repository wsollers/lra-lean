...
/--
**[Theorem - Uniqueness of Integer Structures up to Isomorphism]**

Any two integer structures are isomorphic: there is a bijection between
their carriers that fixes zero and commutes with both successor and
predecessor. The canonical witness sends `0 ↦ 0`, `Sⁿ 0 ↦ Sⁿ 0`, and
`Pⁿ 0 ↦ Pⁿ 0` on the other structure -- the forward/backward rays generated
from the base -- exactly as `UniquenessOfPeanoSystemsUpToIsomorphism` does
for the one-sided case, per `PeanoSystem.Categoricity`.
