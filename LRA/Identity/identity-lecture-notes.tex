...
\subsection{The Two Directions of Leibniz's Law}

\begin{leanblock}
theorem IndiscernibilityOfIdenticals {x y : Carrier}
    (h : Ident x y) :
    forall Property : Carrier -> Prop, Property x <-> Property y

theorem IdentityOfIndiscernibles {x y : Carrier}
    (h : forall Property : Carrier -> Prop,
      Property x <-> Property y) : Ident x y

theorem IdentLeibnizIff {x y : Carrier} :
    Ident x y <-> forall Property : Carrier -> Prop,
      Property x <-> Property y
\end{leanblock}

The forward direction is the \emph{Indiscernibility of Identicals}: identical
objects satisfy exactly the same predicates. The reverse direction is the
\emph{Identity of Indiscernibles}: objects satisfying exactly the same
predicates are identical. The full theorem packages the two principles as one
biconditional characterization of identity.

Together these theorems form one of the key conceptual bridges in the file.
They turn identity from a relation one can mention into a relation whose entire
mathematical effect is clear: identity prevents every property from
distinguishing two objects, and complete indiscernibility recovers identity.
...
\begin{leanblock}
noncomputable def LRA.Identity.Constructions.Mathlib.CanonicalEqualityStructure
    (Carrier : Type u) [Nonempty Carrier] :
    EqualityStructure where
  Carrier := Carrier
  carrierNonempty := inferInstance
  equalityInterpretation := EqualityDiagonal Carrier
  satisfiesIdentityTheory := by sorry
\end{leanblock}
...
theorem axiomaticIsEq {Carrier : Type u} (x y : Carrier) :
  ...
  @IdentIsDiagonal Carrier
    (LRA.Identity.Constructions.Axiomatic.instIdentityRelation Carrier) x y
\end{leanblock}
