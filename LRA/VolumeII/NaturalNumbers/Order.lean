...
def is_minimum
    (ps : PeanoSystem)
    (P : LRA.VolumeI.Set.LRASet ps.carrier)
    (m : ps.carrier) : Prop :=
  LRA.VolumeI.Set.LRASet.member m P ∧
    ∀ k : ps.carrier, LRA.VolumeI.Set.LRASet.member k P → ¬ lt ps k m
...
theorem well_ordering
    (ps : PeanoSystem)
    (P : LRA.VolumeI.Set.LRASet ps.carrier)
    (P_nonempty : LRA.VolumeI.Set.LRASet.nonempty P) :
