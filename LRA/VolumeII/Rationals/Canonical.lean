...
def equivalent (input : Input)
  ...
  input.integer_model.signature.multiplication
      first.numerator (input.to_integer second.denominator) =
    input.integer_model.signature.multiplication
      second.numerator (input.to_integer first.denominator)

/-- Theorem 1.3: cross-multiplication is an equivalence relation. -/
theorem equivalent_is_equivalence_relation (input : Input) :
...
def representative_addition (input : Input)
...
def representative_multiplication (input : Input)
    (first second : Representative input) : Representative input where
...
def representative_negation (input : Input)
...
theorem representative_operations_respect_equivalence (input : Input) :
    Foundations.Quotients.binary_operation_respects
        (representative_setoid input) (representative_addition input) ∧
    Foundations.Quotients.binary_operation_respects
        (representative_setoid input) (representative_multiplication input) ∧
    Foundations.Quotients.unary_operation_respects
        (representative_setoid input) (representative_negation input) := by
  sorry
...
theorem negation_exists (input : Input) :
...
def AdditiveGroupLaws (input : Input) : Prop :=
...
theorem additive_group_laws (input : Input) : AdditiveGroupLaws input := by
  sorry

/-- Proposition expressing multiplicative and distributive laws. -/
def MultiplicativeAndDistributiveLaws (input : Input) : Prop :=
  (∀ first second third : Carrier input,
...
theorem reciprocal_exists_uniquely (input : Input)
    (value : Carrier input) (value_nonzero : value ≠ zero input) :
    ∃ reciprocal : Carrier input,
      IsReciprocal input value reciprocal ∧
      ∀ other, IsReciprocal input value other → other = reciprocal := by
  sorry

/-- Definition 2.10: rational reciprocal. -/
noncomputable def inverse (input : Input)
    (value : Carrier input) (value_nonzero : value ≠ zero input) : Carrier input :=
  Classical.choose (reciprocal_exists_uniquely input value value_nonzero)

/-- Theorem 2.11: reciprocal correctness. -/
theorem inverse_is_two_sided (input : Input)
    (value : Carrier input) (value_nonzero : value ≠ zero input) :
    IsReciprocal input value (inverse input value value_nonzero) := by
  sorry

/-- Proposition expressing the field structure. -/
def FieldStructure (input : Input) : Prop :=
  AdditiveGroupLaws input ∧
  MultiplicativeAndDistributiveLaws input ∧
  zero input ≠ one input ∧
  (∀ value : Carrier input,
    value ≠ zero input → ∃ reciprocal, IsReciprocal input value reciprocal)

/-- Theorem 2.12: the rationals form a field. -/
theorem field_structure (input : Input) : FieldStructure input := by
  sorry

/-- Definition 3.1: representative strict order. -/
def representative_strict_order (input : Input)
    (first second : Representative input) : Prop :=
...
theorem representative_strict_order_respects_equivalence (input : Input) :
...
def StrictTotalOrder (input : Input) : Prop :=
  ...
  (∀ first second : Carrier input,
    first ≠ second →
    strict_order input first second ∨ strict_order input second first)

/-- Theorem 3.3: rational strict order is a strict total order. -/
theorem strict_total_order (input : Input) : StrictTotalOrder input := by
  sorry

/-- Proposition expressing ordered-field compatibility. -/
def OrderedFieldCompatibility (input : Input) : Prop :=
  ...
  (∀ first second : Carrier input,
    ...
    strict_order input (zero input) (multiplication input first second))
...
theorem ordered_field_compatibility (input : Input) :
    OrderedFieldCompatibility input := by
  sorry

/-- Proposition expressing ordered-field structure. -/
def OrderedFieldStructure (input : Input) : Prop :=
  FieldStructure input ∧ StrictTotalOrder input ∧ OrderedFieldCompatibility input

/-- Theorem 3.5: the rationals form an ordered field. -/
theorem ordered_field_structure (input : Input) : OrderedFieldStructure input := by
  sorry

/-- Definition 4.1: integer representative with denominator one. -/
def integer_representative (input : Input)
    (value : input.integer_model.signature.carrier) : Representative input where
  numerator := value
  denominator := input.one

/-- Definition 4.1: canonical integer embedding. -/
def integer_embedding (input : Input)
...
theorem integer_embedding_properties (input : Input) :
    ...
    (∀ first second,
      integer_embedding input
        ...
        addition input (integer_embedding input first) (integer_embedding input second)) ∧
    (∀ first second,
      integer_embedding input
          (input.integer_model.signature.multiplication first second) =
        multiplication input (integer_embedding input first) (integer_embedding input second)) ∧
    (∀ first second,
      strict_order input (integer_embedding input first) (integer_embedding input second) ↔
        input.integer_model.signature.strict_order first second) := by
  sorry

/-- Definition 5.1: a representative is reduced when gcd(|a|,b)=1. -/
def IsReduced (input : Input) (representative : Representative input) : Prop :=
  input.gcd
      (input.absolute_numerator representative.numerator)
      representative.denominator = input.one

/-- Theorem 5.2: every rational has a reduced representative. -/
theorem reduced_representative_exists (input : Input) (value : Carrier input) :
    ∃ representative : Representative input,
      IsReduced input representative ∧ Quotient.mk _ representative = value := by
  sorry

/-- Theorem 5.3: reduced representatives are unique. -/
theorem reduced_representative_unique (input : Input)
...
theorem unique_lowest_term_form (input : Input) (value : Carrier input) :
    ∃ representative : Representative input,
...
theorem density (input : Input)
    (first second : Carrier input)
    (first_lt_second : strict_order input first second) :
    ∃ middle,
      strict_order input first middle ∧ strict_order input middle second := by
  sorry

/-- Theorem 6.2: Archimedean property of the rationals. -/
theorem archimedean_property (input : Input) (value : Carrier input) :
...
theorem no_rational_square_root_two (input : Input) (two : Carrier input) :
    ¬ ∃ value, multiplication input value value = two := by
  sorry

/-- Theorem 7.4: the square-root cut has no rational supremum. -/
theorem square_root_two_cut_has_no_supremum (input : Input)
    (two : Carrier input) :
    ¬ ∃ supremum,
      (∀ value,
        square_root_two_cut input two value →
...
def NotOrderComplete (input : Input) : Prop :=
  ∃ subset : Carrier input → Prop,
    (∃ value, subset value) ∧
    (∃ upper,
      ∀ value, subset value → nonstrict_order input value upper) ∧
    ¬ ∃ supremum,
      (∀ value, subset value → nonstrict_order input value supremum) ∧
      (∀ upper,
        (∀ value, subset value → nonstrict_order input value upper) →
        nonstrict_order input supremum upper)

/-- Corollary 7.5: the rationals are not order-complete. -/
theorem not_order_complete (input : Input) : NotOrderComplete input := by
  sorry

/-- Proposition expressing the final structural summary. -/
def StructureOfTheRationals (input : Input) : Prop :=
  OrderedFieldStructure input ∧
  (∀ first second : Carrier input,
    strict_order input first second →
    ∃ middle,
      strict_order input first middle ∧ strict_order input middle second) ∧
  NotOrderComplete input

/-- Theorem 8.1: final structural summary. -/
theorem structure_of_the_rationals (input : Input) :
    StructureOfTheRationals input := by
  sorry
