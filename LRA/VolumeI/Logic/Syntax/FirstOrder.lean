...
the collection of all (free and bound) variables (`allVariables`), and the
subformula relation (`IsSubformula`). The substitution lemma
connecting `substitute` to `Satisfies` is added here once proved.
-/
