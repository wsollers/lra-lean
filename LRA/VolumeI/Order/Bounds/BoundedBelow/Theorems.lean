...
/--
`SymmetricDifferenceBoundedBelow`

Statement: A symmetric difference is bounded below when both operands are.

Logical form: `BoundedBelow r A → BoundedBelow r B → BoundedBelow r (A ∆ B)`.
-/
