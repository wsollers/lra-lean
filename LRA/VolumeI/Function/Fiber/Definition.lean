...
The predicate fiber of a typed function over a codomain element.
...
A backend set is the fiber of a typed function over an output when its members are
...
The backend set fiber of a typed function over an output.
...
The kernel relation of a typed function: two inputs are related when the map sends
...
def KernelRelation {Domain : Type u} {Codomain : Type v}
    (map : Function Domain Codomain) : Domain -> Domain -> Prop
```
-/
def KernelRelation {Domain : Type u} {Codomain : Type v}
    (map : Function Domain Codomain) : Domain -> Domain -> Prop :=
