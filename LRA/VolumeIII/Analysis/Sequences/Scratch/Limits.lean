...
`ultimately_constant_sequence_convergence` from
...
theorem eventual_strict_comparison_preserves_weak_limit_order_gt
    {x y : RealSequence} {A B : ℝ} (hA : ConvergesTo x A)
    (hB : ConvergesTo y B) (h : ∃ N : ℕ, ∀ n ≥ N, x n > y n) : A ≥ B := by
  sorry

/-- `thm:constant-comparison-sequence-limits`, part (a). -/
theorem constant_comparison_sequence_limits_le {x : RealSequence} {A B : ℝ}
    (hA : ConvergesTo x A) (h : ∃ N : ℕ, ∀ n ≥ N, x n ≤ B) : A ≤ B := by
  sorry

/-- `thm:constant-comparison-sequence-limits`, part (b). -/
theorem constant_comparison_sequence_limits_lt {x : RealSequence} {A B : ℝ}
    (hA : ConvergesTo x A) (h : ∃ N : ℕ, ∀ n ≥ N, x n < B) : A ≤ B := by
  sorry

/-- `thm:constant-comparison-sequence-limits`, part (c). -/
theorem constant_comparison_sequence_limits_ge {x : RealSequence} {A B : ℝ}
    (hA : ConvergesTo x A) (h : ∃ N : ℕ, ∀ n ≥ N, x n ≥ B) : A ≥ B := by
  sorry

/-- `thm:constant-comparison-sequence-limits`, part (d). -/
theorem constant_comparison_sequence_limits_gt {x : RealSequence} {A B : ℝ}
    (hA : ConvergesTo x A) (h : ∃ N : ℕ, ∀ n ≥ N, x n > B) : A ≥ B := by
  sorry

/-- `thm:constant-squeeze-theorem`. -/
theorem constant_squeeze_theorem {x : RealSequence} {L : ℝ}
    (h : ∃ N₀ : ℕ, ∀ n ≥ N₀, L ≤ x n ∧ x n ≤ L) : ConvergesTo x L := by
  sorry

/-- `thm:sequence-squeeze-theorem`. -/
theorem sequence_squeeze_theorem {a x b : RealSequence} {L : ℝ}
    (ha : ConvergesTo a L) (hb : ConvergesTo b L)
    (h : ∃ N₀ : ℕ, ∀ n ≥ N₀, a n ≤ x n ∧ x n ≤ b n) : ConvergesTo x L := by
  sorry

/-- `thm:absolute-value-squeeze-theorem`. -/
theorem absolute_value_squeeze_theorem {x u : RealSequence} {L : ℝ}
...
the two bounds. -/
theorem convergent_sequence_is_bounded {x : RealSequence} {L : ℝ}
