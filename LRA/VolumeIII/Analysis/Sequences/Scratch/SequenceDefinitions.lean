...
`LRA.VolumeIII.Analysis.Bounding.Scratch.*` files from the earlier Bounds
...
abbrev Sequence (X : Type*) [Nonempty X] : Type _ := ℕ → X
...
theorem reciprocal_sequence_converges_to_zero :
...
theorem natural_number_sequence_diverges :
