...
theorem factorial_partial_sums_approximate_e :
...
by the Lean-`0` index. -/
def CompoundInterestSeq : RealSequence :=
  fun n => (1 + 1 / ((n : ℝ) + 1)) ^ (n + 1)

/-- `thm:compound-interest-approximation-e`. -/
theorem compound_interest_approximation_e :
    ConvergesTo CompoundInterestSeq (Real.exp 1) := by
  sorry

/-- `def:decimal-truncation-sequence`. -/
def DecimalTruncationSeq (α : ℝ) : RealSequence :=
  fun n => (⌊10 ^ n * α⌋ : ℝ) / 10 ^ n

/-- `thm:decimal-truncations-converge`. -/
theorem decimal_truncations_converge (α : ℝ) :
