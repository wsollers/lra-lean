...
theorem subsequence_indices_dominate_identity {σ : ℕ → ℕ}
    (h : IsStrictlyIncreasingIndexMap σ) : ∀ k, k ≤ σ k := by
  sorry

/-- `thm:subsequences-preserve-limits`. -/
theorem subsequences_preserve_limits {x : RealSequence} {L : ℝ} {σ : ℕ → ℕ}
...
theorem subsequential_limit_of_convergent_sequence {x : RealSequence}
    {L K : ℝ} (hL : ConvergesTo x L) (hK : IsSubsequentialLimit x K) :
    K = L := by
  sorry

/-- `thm:divergence-by-two-subsequential-limits`. -/
theorem divergence_by_two_subsequential_limits {x : RealSequence}
...
theorem boundedness_passes_to_subsequences {x : RealSequence} {σ : ℕ → ℕ}
...
theorem monotonicity_passes_to_subsequences_incr {x : RealSequence}
...
theorem monotonicity_passes_to_subsequences_decr {x : RealSequence}
    {σ : ℕ → ℕ} (hx : IsDecreasing x) (hσ : IsStrictlyIncreasingIndexMap σ) :
    IsDecreasing (fun k => x (σ k)) := by
  sorry

/-- `thm:subsequence-of-subsequence`. -/
theorem subsequence_of_subsequence {x : RealSequence} {σ τ : ℕ → ℕ}
...
`x_n`). -/
theorem eventual_properties_pass_to_subsequences {P : ℕ → Prop} {σ : ℕ → ℕ}
    (h : ∃ N : ℕ, ∀ n ≥ N, P n) (hσ : IsStrictlyIncreasingIndexMap σ) :
    ∃ K : ℕ, ∀ k ≥ K, P (σ k) := by
  sorry

/-- `thm:frequent-properties-yield-subsequences`. -/
theorem frequent_properties_yield_subsequences {P : ℕ → Prop}
    (h : ∀ N : ℕ, ∃ n ≥ N, P n) :
    ∃ σ : ℕ → ℕ, IsStrictlyIncreasingIndexMap σ ∧ ∀ k, P (σ k) := by
  sorry

/-- `thm:subsequential-limits-respect-bounds`. -/
theorem subsequential_limits_respect_bounds {x : RealSequence} {L m M : ℝ}
...
theorem squeeze_passes_to_subsequences {a x b : RealSequence} {σ : ℕ → ℕ}
...
theorem monotone_subsequence_theorem (x : RealSequence) :
...
theorem bolzano_weierstrass_sequences {x : RealSequence} (h : BoundedSeq x) :
    HasConvergentSubsequence x := by
  sorry

/-- `thm:sequential-compactness-closed-bounded-interval`. -/
theorem sequential_compactness_closed_bounded_interval {a b : ℝ} {x : RealSequence}
...
`subsequences_preserve_limits` with `τ = id`; backward direction is by
contradiction via `frequent_properties_yield_subsequences` (thread an
...
theorem subsequence_principle {x : RealSequence} {L : ℝ} :
    ConvergesTo x L ↔
      ∀ σ : ℕ → ℕ, IsStrictlyIncreasingIndexMap σ →
...
`bolzano_weierstrass_sequences` (needs boundedness — without it,
"unique subsequential limit" is vacuous for a sequence with none, e.g.
`x_n = n`); uniqueness rules out a second one via
`divergence_by_two_subsequential_limits`. Both ingredients already live
in this file. -/
theorem bounded_sequence_converges_iff_unique_subsequential_limit
