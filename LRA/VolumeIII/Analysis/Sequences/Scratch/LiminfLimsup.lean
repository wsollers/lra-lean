...
accounted for: 2 defs (tail-supremum/-infimum sequence) and 1 theorem
...
def TailSupSeq (x : RealSequence) : RealSequence :=
  fun n => sSup (x '' {k : ℕ | n ≤ k})

/-- `def:tail-infimum-sequence`. -/
def TailInfSeq (x : RealSequence) : RealSequence :=
...
theorem tail_suprema_are_decreasing {x : RealSequence} (h : BoundedSeq x) :
    IsDecreasing (TailSupSeq x) := by
  sorry

/-- `thm:tail-infima-are-increasing`. -/
theorem tail_infima_are_increasing {x : RealSequence} (h : BoundedSeq x) :
    IsIncreasing (TailInfSeq x) := by
  sorry

/-- `thm:liminf-below-limsup`. -/
theorem liminf_below_limsup {x : RealSequence} (hbdd : BoundedSeq x)
    {I S : ℝ} (hI : LiminfSeq x I) (hS : LimsupSeq x S) : I ≤ S := by
  sorry

/-- `thm:convergence-iff-liminf-equals-limsup`. -/
theorem convergence_iff_liminf_equals_limsup {x : RealSequence}
    (hbdd : BoundedSeq x) {L : ℝ} :
    ConvergesTo x L ↔ (LiminfSeq x L ∧ LimsupSeq x L) := by
  sorry

/-- `thm:limsup-largest-subsequential-limit`. -/
theorem limsup_largest_subsequential_limit {x : RealSequence}
...
theorem liminf_smallest_subsequential_limit {x : RealSequence}
...
theorem oscillation_criterion_via_liminf_limsup {x : RealSequence}
...
theorem limsup_comparison_under_eventual_order {x y : RealSequence}
...
theorem liminf_comparison_under_eventual_order {x y : RealSequence}
    (hx : BoundedSeq x) (hy : BoundedSeq y)
...
theorem limsup_squeeze_under_eventual_order {a x b : RealSequence}
...
theorem liminf_squeeze_under_eventual_order {a x b : RealSequence}
    (ha : BoundedSeq a) (hx : BoundedSeq x) (hb : BoundedSeq b)
    (h : ∃ N : ℕ, ∀ n ≥ N, a n ≤ x n ∧ x n ≤ b n)
...
function equation. -/
theorem limsup_neg_eq_neg_liminf {x : RealSequence} {L : ℝ} :
    LimsupSeq (fun n => -x n) L ↔ LiminfSeq x (-L) := by
  sorry

/-- ADDITION (not a `.tex` label — see `ADDITIONS.md` item 8). Dual of
`limsup_neg_eq_neg_liminf`: `liminf(-x_n) = -limsup(x_n)`. Follows from
...
theorem liminf_neg_eq_neg_limsup {x : RealSequence} {L : ℝ} :
