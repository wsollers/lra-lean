...
def PointwiseQuotient (f g : ℝ → ℝ) : ℝ → ℝ := fun x => f x / g x

/-- `def:pointwise-reciprocal-of-a-function`. -/
def PointwiseReciprocal (f : ℝ → ℝ) : ℝ → ℝ := fun x => 1 / f x
...
theorem function_algebra_closure (f g : ℝ → ℝ) (lam : ℝ) (A : Set ℝ) :
...
"is not a function." -/
theorem quotient_undefined_when_denominator_vanishes (f g : ℝ → ℝ)
