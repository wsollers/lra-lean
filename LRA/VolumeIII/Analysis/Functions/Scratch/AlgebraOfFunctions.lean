...
function-theory content, no dependency on the Sequences/Bounding scratch
...
theorem composition_injective {f : A → B} {g : B → C}
...
theorem preimage_union_intersection {f : A → B} (S T : Set B) :
    f ⁻¹' (S ∪ T) = f ⁻¹' S ∪ f ⁻¹' T ∧
      f ⁻¹' (S ∩ T) = f ⁻¹' S ∩ f ⁻¹' T ∧
      f ⁻¹' Sᶜ = (f ⁻¹' S)ᶜ := by
  sorry
