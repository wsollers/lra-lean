...
theorem bounded_away_from_zero_implies_nonzero (f : ℝ → ℝ) (A : Set ℝ)
    (h : FunctionBoundedAwayFromZero f A) : ∀ x ∈ A, f x ≠ 0 := by
  sorry

/-- `prop:bounded-away-from-zero-near-implies-nonzero-near`. -/
theorem bounded_away_from_zero_near_implies_nonzero_near (f : ℝ → ℝ)
...
theorem bounded_away_from_zero_gives_bounded_reciprocal (f : ℝ → ℝ) (A : Set ℝ)
...
theorem local_bounded_away_from_zero_gives_local_bounded_reciprocal
...
theorem uniform_boundedness_transfer_under_pointwise_limit
...
theorem bounded_function_algebra_closure (f g : ℝ → ℝ) (A : Set ℝ) (lam : ℝ)
...
/-- `cor:bounded-sum`. -/
theorem bounded_sum (f g : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) (hg : FunctionBounded g A) :
    FunctionBounded (fun x => f x + g x) A := by
  sorry

/-- `cor:bounded-difference`. -/
theorem bounded_difference (f g : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) (hg : FunctionBounded g A) :
    FunctionBounded (fun x => f x - g x) A := by
  sorry

/-- `cor:bounded-product`. -/
theorem bounded_product (f g : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) (hg : FunctionBounded g A) :
    FunctionBounded (fun x => f x * g x) A := by
  sorry

/-- `cor:bounded-scalar-multiple`. -/
theorem bounded_scalar_multiple (f : ℝ → ℝ) (A : Set ℝ) (lam : ℝ)
    (hf : FunctionBounded f A) : FunctionBounded (fun x => lam * f x) A := by
  sorry

/-- `cor:bounded-absolute-value`. -/
theorem bounded_absolute_value (f : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) : FunctionBounded (fun x => |f x|) A := by
  sorry

/-- `cor:bounded-max-min`. -/
theorem bounded_max_min (f g : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) (hg : FunctionBounded g A) :
    FunctionBounded (fun x => max (f x) (g x)) A ∧
      FunctionBounded (fun x => min (f x) (g x)) A := by
  sorry
...
theorem bounded_functions_form_commutative_ring (A : Set ℝ) :
...
theorem bounded_functions_form_real_vector_space (A : Set ℝ) :
...
theorem bounded_away_quotient (f g : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) (hg : FunctionBoundedAwayFromZero g A) :
    FunctionBounded (fun x => f x / g x) A := by
  sorry

/-- `cor:bounded-quotient-by-bounded-away-denominator`. Identical
statement to `bounded_away_quotient` above — see header note. -/
theorem bounded_quotient_by_bounded_away_denominator (f g : ℝ → ℝ) (A : Set ℝ)
    (hf : FunctionBounded f A) (hg : FunctionBoundedAwayFromZero g A) :
    FunctionBounded (fun x => f x / g x) A := by
  sorry
