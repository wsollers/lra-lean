...
Schema: \texttt{lra.lean-extraction-contract/v4}\\
Manifest: \texttt{lra.pilot.supremum-harvest/1.1}\\
Primary: \texttt{LeastUpperBound}\\
Seeds: \textbf{36}; dependency-closed declarations: \textbf{46}; typed edges: \textbf{268}.
\end{leanbox}
\small
...
\end{leanbox}
\scriptsize
Policy: \texttt{lra.pilot.logical-unfolding/1.1}. Unfolded:
...
\item \texttt{EmptySetHasNoLeastUpperBoundInReal}: nonemptiness is required for real existence
...
\textbf{Raw extraction logic AST} & available; proposition proof binders still appear as FORALL & not applicable & Lean transform fix \\
...
\textbf{Examples} & 8 checked manifest examples & figures/exposition, no formal attachment & Authored selection + checked witnesses \\
\textbf{Counterexamples} & 5 checked records & empty-set explanation present & Authored selection + checked witnesses \\
\textbf{Formal dependencies} & typed graphs available (268 selected-scope edges) & not shown & Lean \\
