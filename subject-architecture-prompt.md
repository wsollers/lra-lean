...
LRA.Interop.Mathlib   durable Mathlib interoperability
...
| `Interop` | adapters to Mathlib or to Lean core |
...
Exactly these leaf filenames. Do not reintroduce the older per-concept sprawl.
...
`Definition.lean`, `Theorems.lean`, or router may import them. See §6.

### 2.4 Volume aggregators
...
## 4. Doc-comment standard

Every `def`, `abbrev`, `structure`, `class`, `inductive`, `theorem`, and `axiom`
carries a `/-- … -/` doc-comment whose first line states the mathematical
content in prose.

**Settle the display-name question.** Only 6 of 311 promoted files use the
`**[Definition — Name]**` marker, while `scripts/lint_doccomments.sh` still
requires it. Pick one and make code and linter agree:

- *Recommended*: drop the marker. The declaration name is already in the
  signature, the marker duplicates it, and the promoted subjects have already
  abandoned it. Update `scripts/lint_doccomments.sh` to require a doc-comment
  and a non-empty first prose line instead.
- If the marker is kept for blueprint generation, say so in the linter and apply
  it to all 311 promoted files plus the 974 Volume II declarations currently
  missing it.

Do not emit a "Logical form:" code block that restates the signature verbatim.
It duplicates the declaration and drifts out of date. Where a doc-comment must
show a form the signature cannot, show only the part that adds information.

Module-level `/-! … -/` comments are required on every router and explain what
the group owns and what it deliberately does not.
...
| Volume VII | `LRA/VolumeVII/` | `LRA/Interop/Mathlib/`, `LRA.Internal` |
...
- Every declaration has a doc-comment; code and `lint_doccomments.sh` agree.
